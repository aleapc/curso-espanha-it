...
// união — trocar nos dois sentidos deixa os aparelhos iguais. Prefixo: IT1.
...
export function encodeSync(): string {
  ...
  return 'IT1.' + b64uEncode(JSON.stringify(payload));
}
...
export function decodeSync(code: string): SyncData | null {
  try {
    ...
    const m = code.trim().match(/IT1.([A-Za-z0-9_-]+)/);
    if (!m) return null;
...
export function whatsappUrl(code: string): string {
  ...
  const text =
    `📚 *¡Dime!* — i miei progressi!\n` +
    `Copia il codice e incollalo nell'app ¡Dime! (Home → Sync → Incolla):\n\n` +
    `${code}\n\n` +
    `(oppure apri ${shareUrl(code)} — ti guida)`;
