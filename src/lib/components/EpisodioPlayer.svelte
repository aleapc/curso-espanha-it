<script lang="ts">
  ...
  let modo = $state<'estudo' | 'carro'>(
    typeof localStorage !== 'undefined' && localStorage.getItem('it-modo') === 'carro'
      ? 'carro'
      : 'estudo'
  );
  function setModo(m: 'estudo' | 'carro') {
    modo = m;
    if (typeof localStorage !== 'undefined') localStorage.setItem('it-modo', m);
  }
  ...
  function setFator(f: number) {
    fator = f;
    if (typeof localStorage !== 'undefined') localStorage.setItem('it-pausa', String(f));
  }
