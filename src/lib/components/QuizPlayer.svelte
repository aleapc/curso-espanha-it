...
<div class="card p-4">
  ...
  <div class="mt-3 flex flex-wrap items-center gap-2">
    <button type="button" class="btn-primary" onclick={escutar}>
      {tocando ? '⏸ Pause' : emMeio || pos > 0 ? '▶ Weiter' : '▶ Gespräch anhören'}
    </button>
    <button
      type="button"
      ...
      <p class="mt-1 text-carvao/70">
        {acertos === quiz.perguntas.length
          ...
          : acertos >= quiz.perguntas.length - 2
            ? 'Gut gemacht. Fast alles. 👏'
            : 'Guter Anfang — hör nochmal rein und probier es wieder. 💪'}
      </p>
      <div class="mt-4 flex justify-center gap-2">
        <button type="button" class="btn bg-white ring-1 ring-black/10" onclick={refazer}>Nochmal</button>
