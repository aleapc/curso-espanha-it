...
// AS VOZES-GUIA DESTE SKU. Este set é a linha inteira que separa ativo de PAR
// (a narração, que se refaz a cada comprador) de ativo de DESTINO (a fala
// espanhola, que se reusa). Copiado do curso inglês ele dizia Alice/George — e
// com esse set a narração ALEMÃ seria contada como ativo de destino, isto é, o
// script informaria que o quinto SKU sai de graça. É a conta em que todo o
// catálogo se apoia, invertida em silêncio. `Bia` fica porque o collect-audio
// ainda a usa como voz-guia default de step que não declara `voz`.
const GUIA = new Set(['Juli', 'Thomas', 'narrador', 'Bia']);
